...
{
    public partial class RdpConnection : Entity<RdpConnection>
    {
        #region 对象操作
        ...
        public static IReadOnlyList<RdpConnection> FindAllByIds(IReadOnlyCollection<Guid> ids)
        {
            if (ids.Count == 0)
            {
                return Array.Empty<RdpConnection>();
            }
            Expression where = @_.Id.In(ids);
            return Entity<RdpConnection>.FindAll(where).ToList();
        }
        #endregion
